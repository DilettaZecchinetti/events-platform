...
const ManualEventCard = ({
    ...
    // Determine if this event has already ended
    const isPastEvent =
            ...
            <Card.Body className="d-flex flex-column justify-content-between">
                <div>
                    ...
                    <Card.Text
                        ...
                        style={{ fontSize: "1rem", fontStyle: "italic", margin: "0.25rem 0" }}
                    >
                        {event.description}
                    </Card.Text>

                    <Card.Text className="text-secondary" style={{ marginTop: "0.5rem" }}>
                        📍 {event.location?.venue}, {event.location?.city}
                    </Card.Text>
                </div>
                ...
                <div className="mt-3 d-flex flex-column gap-2">
                    <Button
                        variant={isSignedUp ? "secondary" : "primary"}
                        onClick={() => handleSignUp(event._id)}
                        disabled={signupLoading[event._id] || isSignedUp || isPastEvent}
                    >
                        {isPastEvent
                            ? "Event Ended"
                            : signupLoading[event._id]
                    ...
                    <Button
                        variant={calendarAdded[event._id] ? "secondary" : "success"}
                        onClick={() => handleAddToCalendar(event)}
                        disabled={calendarLoading[event._id] || calendarAdded[event._id] || isPastEvent}
                    >
                        {isPastEvent
                            ? "Event Ended"
                            : calendarLoading[event._id]
